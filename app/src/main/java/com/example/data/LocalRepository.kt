...
class LocalRepository(val db: AppDatabase, val context: android.content.Context) {
    ...
    private fun validateAndSanitizeJournalEntry(entry: JournalEntry): JournalEntry {
        ...
        if (trimmedTitle.isEmpty()) {
            throw IllegalArgumentException("Database Integrity Error: Journal title cannot be blank.")
        }
        if (trimmedText.isEmpty()) {
            throw IllegalArgumentException("Database Integrity Error: Journal content text cannot be blank.")
        }
